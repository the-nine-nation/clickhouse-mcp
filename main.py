...
# 直接从lifespan_code模块导入app_context和DatabaseConnection，确保使用同一个实例
from clickhouse_mcp.lifespan_code import (
    app_lifespan, DatabaseConnection, app_context, DB_CONFIG,
    CLICKHOUSE_NATIVE_AVAILABLE, CLICKHOUSE_HTTP_AVAILABLE
)
from clickhouse_mcp.func import execute_http_query, process_native_result, format_query_results
...
async def clickhouse_execute_read(sql_query: Annotated[str, Field(description="The SQL code to execute (only SELECT, SHOW, DESCRIBE, EXPLAIN allowed)")],
                                 ...
                                 params: Annotated[Optional[Dict[str, Any]], Field(description="Parameters for the SQL query (for parameterized queries)", default=None)] = None,
                                 explanation: Annotated[Optional[str], Field(description="One sentence explanation as to why this tool is being used, and how it contributes to the goal.")] = None) -> str:
    """
...
def get_connection() -> Optional[DatabaseConnection]:
    ...
    # 确保连接是活跃的
    if app_context.connection and app_context.connection_mode:
        logger.info(f"使用{app_context.connection_mode}模式连接")
...
async def execute_db_query(query: str, params: dict, max_rows: int) -> str:
    """执行数据库查询并返回结果"""
    try:
        # 安全检查：只允许SELECT、SHOW、DESCRIBE、DESC、EXPLAIN等只读操作
        ...
        # 处理特殊表名 - 如果是DESCRIBE语句，检查表名中是否有点号，有则需要引号包围
        if query_lower.startswith(("describe", "desc")):
            try:
                ...
                if len(parts) > 1:
                    ...
                    # 如果表名中包含点号且不是以引号开头，则添加反引号
                    if '.' in table_name and not (table_name.startswith('`') and table_name.endswith('`')):
                        # 处理schema.table形式
                        schema_table = table_name.split('.')
                        if len(schema_table) == 2:
                            # 保留schema，给table部分加引号
                            schema, table = schema_table
                            if table.startswith('.'):
                                # 处理特殊表名如 ueba..inner_id.xxx 
                                query = f"{parts[0]} {schema}.`{table}`"
                            else:
                                query = f"{parts[0]} {schema}.`{table}`"
                        else:
                            # 整个表名加引号
                            query = f"{parts[0]} `{table_name}`"
                        logger.info(f"重写的查询: {query}")
            except Exception as e:
                logger.warning(f"处理特殊表名时出错: {e}")
                # 继续执行原始查询，让数据库报错
